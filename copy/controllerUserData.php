...
if(isset($_POST['signup'])){
    ...
    if(count($errors) === 0){
        ...
        $status = "not verified";
        $insert_data =  "INSERT INTO `student`( `SSn`, `SName`, `SEmail`, `SPhone`, `SPassword`, `Sfile`, `code`, `status`, `a_ID`) VALUES
        ('$ssn','$name','$email','$phone','$encpass','$file','$code','$status','1101708566')";
...
$res = mysqli_query($con, $ssn_check);
if(mysqli_num_rows($res) > 0){
    $errors['student_id'] = "رقم الهوية الذي أدخلته موجود بالفعل!";
}
...
if(count($errors) === 0){
    ...
    $status = "not verified";
    $insert_data =  "INSERT INTO `student`( `SSn`, `SName`, `SEmail`, `SPhone`, `SPassword`, `Sfile`, `code`, `status`, `a_ID`) VALUES
    ('$ssn','$name','$email','$phone','$pass','$file','$code','$status','1101708566')";
   $data_check = mysqli_query($con, $insert_data);
   ...
   if($data_check){
    ...
    $sender = "from :hanan.almimoni@gmail.com";
        if(mail($email, $subject, $message, $sender)){
            ...
            header('location: allstudentt.php');
            exit();
        }else{
            $errors['otp-error'] = "خطأ اثاء ارسال رقم التحقق";}
    }else{$errors['db-error'] = "خطأ اثناء اضافة البيانات إلى قاعدة البيانات";
    }}
}
////////////////////////////////
